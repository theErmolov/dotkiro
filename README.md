...
Sync your team's [Kiro](https://kiro.dev) steering files, skills, and custom agents from a central Git repo into any project.
...
This pulls all `.md` files from the `steering/` and `skills/` directories and all `.md` and `.json` files from the `agents/` directory at the root of your conventions repo into `.kiro/steering/`, `.kiro/skills/`, and `.kiro/agents/`.
...
This fetches files from `python/steering/`, `python/skills/`, and `python/agents/` in the remote repo without re-syncing the shared ones. See [Repo structure](#repo-structure) for the full layout.

## Commands

### `dotkiro init [types...]`

Fetches steering files, skills, and agents from the configured remote repo and copies them into `.kiro/steering/`, `.kiro/skills/`, and `.kiro/agents/`.

- With no types and no `types` in `.dotkirorc`, syncs only the shared conventions (files at the repo root's `steering/`, `skills/`, and `agents/` directories).
...
`dotkiro init` shallow-clones your configured repo and copies files into `.kiro/steering/`, `.kiro/skills/`, and `.kiro/agents/`. Steering and skills are `.md` files; agents can be either `.md` (markdown subagents) or `.json` (CLI agent configs).
...
Files you create manually in `.kiro/steering/`, `.kiro/skills/`, or `.kiro/agents/` are never in the manifest, so dotkiro will never touch them. Only files that came from the central repo are tracked and managed.
...
Steering supports subfolders in Kiro, so type-specific steering goes into `.kiro/steering/<type>/`. Skills only work one level deep, so they're flattened into `.kiro/skills/` regardless of source. Agents are flat files in `.kiro/agents/` too, so type-specific agents are flattened there alongside the shared ones.

### Local files are safe

Files you create locally in `.kiro/steering/`, `.kiro/skills/`, or `.kiro/agents/` are never touched by dotkiro. Only files tracked in the manifest are managed.
...
**"No files found in the configured paths"** — The remote repo exists but has no files in the expected `steering/`, `skills/`, or `agents/` directories. Check your [repo structure](#repo-structure).

**"No files found for \<type\>"** — The type folder doesn't exist in the remote repo, or it has no `steering/`, `skills/`, or `agents/` subdirectories with matching files.
